//// TO DO: BUILD BASIC EDGE BINDING MODE FOR THE 
...
void setup() {
  ...
  E_Ref.circle(width/2, height/2, outerDiam);
  E_Ref.circle (width/2, height/2, outerDiam+40);
  PEmbroiderStart();
  E.stroke(255, 0, 0);
  E.stroke(0, 0, 255);
  drawDoublMossyCirlce(E,0, 0, 1, 200);
  E.circle(width/2, height/2, outerDiam);
  ////

  E.visualize(true, true, true);
  E_Ref.visualize(true, true, true);
...
void drawDoublMossyCirlce(PEmbroiderGraphics E, int x, int y, float z, int diamOffset) {
  PVector centOff = new PVector(random(-80, 80), random(-80, 80));
  E.stroke(int(random(255)), int(random(255)), int(random(255)));
  drawMossyCircle(E, x, y, 40, z, 40, 2, centOff, diamOffset, true);
  drawMossyCircle(E, x, y, 40, z, 35, 2, centOff, diamOffset, true);
  drawMossyCircle(E, x, y, 40, z, 0, 4, centOff, diamOffset, false);
  drawMossyCircle(E, x, y, 40, z, 10, 2, centOff, diamOffset, true);
}


///////////////////
void drawMossyCircle(PEmbroiderGraphics E, int x, int y, int diam2, float z, float zigOffset, float arLen, PVector centOff, int diamOffset, boolean trace) {
  float filterTest = 4;
  ...
  for (int j = 0; j<loops; j++) {
    ...
    if (!trace) {
      ...
      E.beginShape();
      while (thSteps < PI*2+theta) {
        int offsetVal2 = int(noiseLoop(1, (thSteps+thStep)/(2*PI), z)*offset*offsetMultiplier(diam2)); // INTERIOr
        PVector P2 = pol2cart(diam2+offsetVal2+intOffset, thSteps+thStep); // INT
        E.vertex(P2.x, P2.y);
        thSteps += thStep;
      }
      E.endShape();
      ...
      E.beginShape();
      while (thSteps < PI*2+theta) {
        int offsetVal1 = int(noiseLoop(1, thSteps/(2*PI), z+1)*offset*offsetMultiplier(diam1)); // EXTERIOR
        PVector P1 = pol2cart(diam1+offsetVal1-extOffset, thSteps); // EXT
        E.vertex(P1.x, P1.y);
        thSteps += thStep;
      }
      E.endShape();
    ...
    diam2 = diam1;
    if(j%2 == 0){
      diam1 += diamOffset/2; /// we make the offset for the non solvent stuff less prominent 
    } else {
      diam1 += diamOffset;
    }
...
void filterInPoly(PEmbroiderGraphics E,int k) {
  int i = E.polylines.size()-1; // looks at the last polyline added
  ArrayList<PVector> collection =  new ArrayList<PVector>();
  for (int j=0; j<E.polylines.get(i).size(); j++) {
    PVector needleLoc0 = E.polylines.get(i).get(j).copy();
    E_Ref.polylines.get(0);
    boolean isInPoly = E_Ref.pointInPolygon(needleLoc0, E_Ref.polylines.get(k));
    if (!isInPoly) {
      collection.add(needleLoc0);
    }
  }
